...
const VideoDetails = () => {
  ...
  const goToPrevVideo = () => {
    ...
    if (currentSubSectionIndx !== 0) {
      ...
      const prevSectionId = courseSectionData[currentSectionIndx - 1]?._id
      const prevSubSections = courseSectionData[currentSectionIndx - 1]?.subSection
  ...
  return (
    <div className="flex flex-col gap-5 text-white">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4">
        ...
        {[0.5, 1, 1.25, 1.5, 2].map((rate) => (
          <button
            ...
            className={`px-4 py-2 ${playbackRate === rate ? "bg-yellow-500" : "bg-gray-700"} rounded`}
          >
            {rate}x
          </button>
        ))}
        ...
        <button onClick={handleFullscreen} className="px-4 py-2 bg-green-600 rounded">
          ⛶ Fullscreen
        </button>
      </div>
      ...
      {videoEnded && (
        <div className="absolute inset-0 z-10 bg-black bg-opacity-80 flex flex-col items-center justify-center gap-4">
          {!completedLectures.includes(subSectionId) && (
            <IconBtn
              disabled={loading}
              onclick={handleLectureCompletion}
